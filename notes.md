...
[dprint.nix](./modules/wrappedPackages/dprint.nix)
...
In case config change is necessary, just use this:

```shell
mkdir -p /tmp/termusic-config
XDG_CONFIG_HOME=/tmp/termusic-config termusic
```

Then make the config changes as necessary, and then you're done. Change the config file as needed.
